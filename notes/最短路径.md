...
## 六、多目标最短路：主目标最短/最省，次目标最大收益（天梯 L2-001 / L3-040）

这类题的套路是：**主目标**用标准最短路保证正确性；在“主目标同样最优”的前提下，再维护 **次目标**（如：路径条数、最大救援队、最大心情值）。

常见维护项：
- `dist[u]`：主目标（最短距离/最小花费）
- `cnt[u]`：到达 u 的最短路径条数（只在主目标等价时累加）
- `best[u]`：在最短距离约束下的最大收益（只在主目标等价时取 max）
- `pre[u]`：恢复一条“最短且收益最大”的路径（当 `best` 被更新时刷新）

松弛规则（核心）：
- 发现更优主目标：全量覆盖（`dist/cnt/best/pre` 都更新）
- 发现等价主目标：
  - `cnt[v] += cnt[u]`
  - 若 `best[u] + gain(v) > best[v]`，更新 `best[v]` 与 `pre[v]`

坑点：
- `cnt` 的累加只发生在 **主目标相等** 的情况下
- `pre` 只为“输出一条路径”服务：当且仅当你决定用这条等价最短路替换为更优收益时才更新

## 七、动态 Floyd：按时间解锁节点的离线查询（洛谷 P1119）

当查询形如“时刻 t 前哪些点/路可用”且 **t 单调推进**（或可被我们强制单调推进）时，可以把 Floyd 的 k 层增量化：

- `times[i]`：节点 i 何时修复可用（已按升序给出）
- 维护指针 `k` 表示当前已解锁的中转点范围是 `[0, k)`  
- 每次收到查询 `(x, y, t)`：
  - `while(k < N && times[k] <= t)`：把这个新解锁的点当作 Floyd 的中转点做一轮松弛，然后 `k++`
  - 若 `times[x] > t` 或 `times[y] > t` 或 `dist[x][y] == INF` → 输出 `-1`
  - 否则输出 `dist[x][y]`

坑点：
- Floyd 的外层必须是 `k`（中转点层），才能保证增量更新的正确性
- 只用 `dist[x][y]` 判定不够：端点未修复时必须判 `times[x]` / `times[y]`
...
| L2-001 | 紧急救援 | L2 | 最短路条数 + 最短路下最大收益（救援队） |
| L2-044 | 大众情人 | L2 | Floyd 全源最短路 + min-max 指标 |
| L3-040 | 心情 | L3 | 最小花费最短路 + 等价主目标下最大心情 |
| P1119 | 灾后重建 | 中等 | 动态 Floyd：按时间解锁中转点离线查询 |
