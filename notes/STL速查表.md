...
### 7. 刷题补充（4.21：L2-058 / L3-043）

- L2-058（超参数搜索）：
    - 模板信号：多次查询“首个大于 x 的值”，且要保留同值最优编号。
    - 落地写法：`map<score, id>` + `insert`（同键不覆盖）+ `upper_bound(x)`。
    - 易错点：题意是严格 `> x`，不是 `>= x`。
- L3-043（门诊预约排队系统）：
    - 模板信号：时间轴事件模拟 + 候选人按预约号有序选择。
    - 落地写法：用 `set` 维护候诊池（按预约号排序），优先命中当前时段预约号，否则按规则选最小预约号。
    - 易错点：ID 需要固定宽度输出（如 `%05d`），以及分支优先级不能写反。
