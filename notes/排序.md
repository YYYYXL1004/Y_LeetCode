...
当有多条有序序列，需要找全局第 K 小时，用小顶堆做多路归并：
- 初始化：每条链表的头元素入堆
- 循环：弹出堆顶（全局最小），将该链表的下一个元素入堆
- 弹出第 K 次即为答案

**关键：堆中始终维护每条链表的当前最小候选，保证每次弹出的都是全局第 i 小。**

#### 有序矩阵第 K 小（378）
- 每行视为一条有序链表，n 行 = n 条链表
- 三元组 `{val, i, j}` 追踪位置，弹出后推入同行下一个元素

#### 查找和最小的 K 对数字（373）
- nums1 的每个元素与整个 nums2 组成一条虚拟有序链表
- 三元组 `{nums1[i], nums2[j], j}` 追踪 nums2 的索引
- 初始只需入堆前 min(nums1.size(), k) 条链表的头（剪枝）
- 弹出后推入同条链表的下一个：`{nums1[i], nums2[j+1], j+1}`
...
while(!pq.empty() && k > 0) {
    auto cur = pq.top(); pq.pop(); k--;
    int next_j = cur[2] + 1;
    if(next_j < nums2.size())
        pq.push({cur[0], nums2[next_j], next_j});
    res.push_back({cur[0], cur[1]});
}
```

**与 23.合并K个升序链表 的关系：** 本质相同，都是多路归并。区别在于 23 题是真实链表节点，373/378 是虚拟链表用索引模拟。
