...
> 有些回溯题不是从数组里“选元素”，而是按位置逐步构造答案，同时维护一个始终合法的中间状态。`22. 括号生成` 就是这类题的典型代表。

### 22. 括号生成

**核心思路**：用 `left`、`right` 表示剩余左/右括号数量，每次尝试放 `'('` 或 `')'`，但任意时刻都必须保证当前前缀合法。

```cpp
void backtrack(int left, int right) {
    if (right < left) return;       // 右括号不够匹配当前前缀
...
**与子集 / 组合题的区别**：
- 选择列表固定为两个字符 `(` 和 `)`，不是遍历一个输入数组
- 剪枝依据不是“去重”，而是“当前构造出来的前缀是否合法”
- 这类题更适合把“约束”直接编码进状态，而不是生成完所有结果后再统一校验
...
这类题不是求“所有解”，而是求一个满足约束的完整棋盘。核心是按格子 DFS，并把规则尽早前置到剪枝里。

**常见约束**：
- 每行每列黑白数量都不能超过一半（6x6 时都不超过 3）
- 任意行/列都不能出现三个连续相同颜色
- 所有行两两不同、所有列两两不同（通常在终态统一校验）
...
bool dfs(int r, int c) {
    ...
    if (board[r][c] != -1) return dfs(nr, nc);

    for (int val = 0; val <= 1; val++) {
        board[r][c] = val;
        if (isValid(r, c) && dfs(nr, nc)) return true;
        board[r][c] = -1;
    }
    return false;
}
...
**实现细节**：
- `isValid(r,c)` 只检查当前行和当前列，避免每步全盘扫描
- 行列“互不相同”可放到最后一次性检查，减少中间状态的字符串构造开销
- 按固定顺序（行优先）推进，代码简单且便于调试
