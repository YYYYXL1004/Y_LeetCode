...
## 题型模板：有序数组窗口排除法
- **适用信号**：答案是有序数组中的连续一段，且每次可以比较左右端点谁更不可能留在答案中
- **通用步骤**：
  1. 初始化 `left = 0, right = n - 1`
  2. 当窗口长度大于目标长度时，比较左右端点与目标的距离或优先级
  3. 删除更差的一端，保持剩余窗口仍然连续
  4. 最终 `[left,right]` 即答案区间
- **复杂度**：时间 `O(n-k)`，空间 `O(1)`（不计返回数组）
- **通用坑点**：平局规则通常决定删哪一侧；如果题目要求结果更小，距离相等时应删右端，保留左侧较小值

## 题目差异：找到 K 个最接近的元素（658）
- **相对模板的变化**：比较标准是 `abs(arr[i]-x)`，距离相同取较小元素
- **本题特化处理**：因为数组有序，答案必然连续；用 `x-arr[left] > arr[right]-x` 判断左端是否更远，平局时删除右端
